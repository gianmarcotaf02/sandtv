  ...
  const renderGroupItem = (group: { id: string; name: string }) => (
    <li key={group.id}>
      <button
        ...
        className={`w-full text-left px-4 py-3 text-sm rounded-2xl transition-all duration-200 transform flex items-center ${
      ...
      <div className="flex-grow overflow-y-auto pr-2 -mr-2">
        ...
        <ul className="space-y-1">
          {customGroups.map((group) => (
            <li key={group.id}>
                <div className={`w-full rounded-2xl flex items-center transition-all duration-200 transform group ${
